...
class EURUSDSTRATEGY:
    def __init__(self, target_pair="EUR/USD"):
        ...
        # TIGHTENED: Risk management for better quality trades
        self.min_rr_ratio = 3.0  # Minimum 1:3 risk-reward ratio
        self.max_rr_ratio = 10.0  # Maximum 1:10 risk-reward ratio
    ...
    def _find_swing_points(self, df, window=5):  # Smaller window for more swings
        highs, lows = [], []
    ...
    def _identify_fibonacci_setup(self, df, current_price):
        ...
        for level_name, level_price in fib_levels.items():
            if 0.236 <= float(level_name.split('_')[1]) <= 0.786:
    ...
    def analyze_trade_signal(self, df: pd.DataFrame, pair: str) -> Dict[str, Any]:
        if len(df) < 50:  # Shorter minimum (was 100)
