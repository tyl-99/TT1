...
class GBPJPYSTRATEGY:
    def __init__(self, target_pair="GBP/JPY"):
        ...
        self.pip_size = 0.01  # GBP/JPY pip size is 0.01 (same as USD/JPY)
        
        # BEST FROM GBP/JPY 50% WIN RATE TUNER: 46.8% win rate, 62 trades
        self.fib_levels = [0.236, 0.382, 0.5]  # Best from 50% win rate tuner
        self.atr_period = 10  # Best from 50% win rate tuner
        self.atr_threshold = 0.25  # Best from 50% win rate tuner
        self.swing_pips = 12  # Best from 50% win rate tuner
        self.risk_reward_ratio = 3.0  # Updated to 1:3 RR
        self.tolerance = 0.0008  # Best from 50% win rate tuner
    ...
    def print_filter_stats(self):
        ...
        if total == 0:
            return
        print(f"\n📊 GBP/JPY FIBONACCI STRATEGY - FILTER STATS:")
